...
export default function UserQuestionTab({ userId, userNama }: { userId: string; userNama: string }) {
  const [questions, setQuestions] = useState<Question[]>(() => getQuestionsForUser(userId));
  const [pertanyaan, setPertanyaan] = useState("");

  useEffect(() => {
    return subscribeToQuestions(() => setQuestions(getQuestionsForUser(userId)));
  }, [userId]);

  const handleAsk = (e: React.FormEvent) => {
    e.preventDefault();
    if (!pertanyaan.trim()) return;
    askQuestion(userId, userNama, pertanyaan.trim());
    setPertanyaan("");
  };
