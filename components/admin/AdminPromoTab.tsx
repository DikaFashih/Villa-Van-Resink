...
export default function AdminPromoTab() {
  const [promo, setPromo] = useState<Promo[]>(() => getAllPromo());
    ...
    <div>
      ...
      {showForm && (
        ...
        >
          <div className="sm:col-span-2">
            <label className="text-xs uppercase tracking-wide text-neutral-500">Paket</label>
            <select
              required
          ...
          <div>
            <label className="text-xs uppercase tracking-wide text-neutral-500">Judul Promo</label>
            <input
              required
          ...
          <div>
            <label className="text-xs uppercase tracking-wide text-neutral-500">Diskon (%)</label>
            <input
              required
              type="number"
              min={1}
          ...
          <div>
            <label className="text-xs uppercase tracking-wide text-neutral-500">Tanggal Mulai</label>
            <input
              required
              type="date"
          ...
          <div>
            <label className="text-xs uppercase tracking-wide text-neutral-500">Tanggal Selesai</label>
            <input
              required
              type="date"
          ...
          <div className="sm:col-span-2">
            <label className="text-xs uppercase tracking-wide text-neutral-500">Deskripsi (opsional)</label>
            <textarea
      ...
      <div className="mt-6 space-y-3">
        {promo.map((p) => {
          ...
          return (
            <div key={p.id} className="flex items-center justify-between rounded-lg border border-[#8A6E4A]/20 bg-white p-4">
              <div>
                <p className="font-medium text-[#23412D]">
                  {p.judul} — {p.diskonPersen}%
                </p>
                <p className="text-xs text-neutral-500">
                  {target?.nama} · Berlaku {p.tanggalMulai} s/d {p.tanggalSelesai}
                </p>
              </div>
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`rounded-full px-3 py-1 text-xs ${
                    p.aktif ? "bg-green-100 text-green-700" : "bg-neutral-100 text-neutral-500"
                  }`}
                >
                  {p.aktif ? "Aktif" : "Nonaktif"}
                </button>
                <button onClick={() => removePromo(p.id)} className="text-red-500 hover:text-red-700">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
        ...
        {promo.length === 0 && <p className="text-sm text-neutral-500">Belum ada promo.</p>}
      </div>
    </div>
  );
}