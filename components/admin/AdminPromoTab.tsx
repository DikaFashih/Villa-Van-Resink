  ...
  return (
    <div>
      ...
      {showForm && (
        <form
          onSubmit={handleAdd}
          className="mt-4 grid gap-3 rounded-lg border border-[#8A6E4A]/20 bg-white p-5 sm:grid-cols-2"
        >
          <select
            required
            value={form.paketSlug}
            onChange={(e) =>
              setForm({ ...form, paketSlug: e.target.value })
            }
          ...
          <textarea
            ...
            className="sm:col-span-2 rounded-md border border-[#8A6E4A]/25 px-3 py-2 text-sm outline-none"
            rows={2}
          />
            ...
            >
              <div>
                ...
                <p className="text-xs text-neutral-500">
                  {target?.nama} · {p.tanggalMulai} s/d {p.tanggalSelesai}
                </p>
              </div>
