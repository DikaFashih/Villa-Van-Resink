...
function RegisterForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setLoading(true);
    const result = register(nama.trim(), email.trim(), password);
    ...
    <section className="flex min-h-screen items-center bg-[#FBF8F2] py-20">
      <Container>
        <div className="mx-auto max-w-md rounded-[16px] border border-[#8A6E4A]/25 bg-white p-8 sm:p-10">
          ...
          <h1 className="mt-2 font-heading text-3xl text-[#23412D]">Buat Akun</h1>
          <p className="mt-2 text-sm text-neutral-600">
            Daftar untuk bisa melakukan booking dan memberi ulasan.
          </p>

          <form onSubmit={handleSubmit} className="mt-8 space-y-4">

            <div>
              <label className="mb-1 block text-xs text-neutral-500">Nama Lengkap</label>
              <input
                type="text"
                required
                value={nama}
            ...
            <div>
              <label className="mb-1 block text-xs text-neutral-500">Konfirmasi Password</label>
              <input
                type="password"
                required
