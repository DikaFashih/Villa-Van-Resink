...
import { getAllRawReviews, removeReview, type Review } from "@/lib/reviews";
...
export default function AdminReviewsTab() {
  ...
  useEffect(() => {
  async function load() {
    const data = await getAllRawReviews();
    setReviews(data);
  }
        ...
        <div key={r.id} className="flex items-start justify-between gap-4 rounded-lg border border-[#8A6E4A]/20 bg-white p-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs uppercase tracking-wide text-[#8A6E4A]">{r.layananNama}</span>
            </div>
